...
def in_any_pocket(ball_name):
    return balls[ball_name].getPosition()[2] < 0.16 or any(pocketed_at(ball_name, pocket) for pocket in POCKETS)
...
def wrong_pocketed():
    owned = role["ball"]
    return in_any_pocket(owned) and not pocketed_at(owned, role["pocket"])
...
def rescue_from_stall(goal):
    ...
    timed_motion(-0.28, role["side_bias"] * 1.8, 0.55)
